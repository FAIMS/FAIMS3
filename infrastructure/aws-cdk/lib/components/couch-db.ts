...
export class EC2CouchDB extends Construct {
  ...
  /**
   * Public HTTPS endpoint for the couch hostname (ALB). When the proxy is
   * enabled this hostname terminates on couch-auth-proxy; otherwise on this
   * instance's target group.
   */
...
EOL`,
    ...
    // DNS ROUTES
    // ===========
    // Public couch.* DNS → shared ALB (proxy or instance TG owns the host rule).
    ...
    // Public hostname (ALB → proxy or this instance, depending on attachToPublicAlb).
