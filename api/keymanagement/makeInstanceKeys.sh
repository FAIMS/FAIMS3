...
cp ./api/couchdb/local.ini.dist ./api/couchdb/local.ini 
sed -i.bak "s/secret = db7a1a86dbc734593febf8ca6fdf0cf8/secret = ${FAIMS_COOKIE_SECRET}/" ./api/couchdb/local.ini
sed -i.bak "s/uuid = adf990d5dd21b735f65d4140ad1f10c2/uuid = "`uuid`"/" ./api/couchdb/local.ini
echo "[jwt_keys]" >> ./api/couchdb/local.ini
echo "rsa:${PROFILE_NAME}=${FLATTENED_KEY}" >> ./api/couchdb/local.ini
echo '[admin]' >> ./api/couchdb/local.ini
echo "admin=${COUCHDB_PASSWORD}" >> ./api/couchdb/local.ini

cat ./api/couchdb/local.ini
