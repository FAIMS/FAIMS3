...
The active version is **`4.0`** (`CURRENT_NOTEBOOK_UI_SCHEMA_VERSION` in `notebookMigrations/index.ts`).
...
| `3.0`                    | `migrateToV4` | Split wire `{ metadata, 'ui-specification' }` into `{ uiSpec, metadata }` with typed `information` / `settings` / `custom`; decode `fviews` → `views`; set `schemaVersion` `4.0` |

`migrateNotebook` runs the chain until the document reaches `4.0`.
...
- **Designer** — saves through the uiSpecification PUT path; exported JSON should target schema `4.0`.
...
1. Add `migrateToV5` (or next) in `notebookMigrations/`, keeping migration output types self-contained in that file (see comment in `migrateV4.ts`).
