...
| Field Type                         | Purpose & Description                                                                                               | Typical Examples                                                                     | Technical Specifications                                                                           | Validation Capabilities                                                      | Platform Considerations                                                  |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| **Number Field**                   | Numeric entry with optional minimum and maximum bounds                                                              | Artefact counts, simple measurements, pH (0–14), percentage (0–100), depth (0–500cm) | Integer or decimal<br/>Optional min/max enforcement<br/>JavaScript number type<br/>Returns: Number | Type checking<br/>Range validation when bounds set<br/>Required field option | Period (.) required for decimals<br/>No locale formatting                |
| **Controlled Number** (deprecated) | Legacy bounded numeric entry (kept for backwards compatibility); use Number Field with min/max bounds for new forms | pH (0–14), percentage (0–100), depth (0–500cm)                                       | Min/max enforcement<br/>Returns: Number                                                            | Range validation<br/>Required field option                                   | Existing fields remain supported; not available in the add-field chooser |
| **Percentage Slider**              | Visual 0–100 style percentage entry                                                                                 | Canopy cover, completion %, rough proportions                                        | Integer values<br/>Configurable min/max (0–100)<br/>Step size<br/>Returns: Integer                 | Range + step grid validation<br/>Required field option                       | Click or drag on track and thumb<br/>Reset clears optional values        |
...
| **DateTime Picker**           | Combined date and time selection with optional one-tap "Now" button                                                        | Observation moments, scheduled records, event timing | ISO 8601 with time<br/>Optional "Now" capture<br/>Returns: DateTime string | Date and time ranges<br/>Required field option | Platform-specific interfaces; recommended date-time field                |
| **DateTime Now** (deprecated) | Legacy one-tap current-timestamp capture (kept for backwards compatibility); use DateTime Picker with "Now" button enabled | Record creation time, observation moment             | Automatic capture<br/>User-triggered<br/>Returns: DateTime string          | Always valid<br/>Can be required               | Existing fields remain supported; not available in the add-field chooser |
...
| **Dropdown (Select) (deprecated)** | Legacy dropdown single selection (kept for backwards compatibility)   | Species list (50+ items), material types     | Conserves screen space<br/>Returns: String value | Required selection<br/>Dependent on vocabulary | Existing fields remain supported; not available in the add-field chooser. Prefer "Radio Buttons" for new forms |
