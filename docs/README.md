...
E2e can emit docs-layout screenshots when `SCREENSHOT_MODE` is `docs` or `all`
(see [e2e/README.md](../e2e/README.md)). The intended destination for Sphinx
user docs is `user/screenshots`, using a naming convention based on the
configured theme (`VITE_THEME`) and screen size (mobile, tablet or desktop):

```text
{{theme}}/{{tag}}/image-name-{{screen size}}.png
```

Copying/syncing those shots into the Sphinx tree is still a manual follow-up;
run artifacts also live under `e2e/artifacts/` for CI review galleries.
