...
class WelcomePage extends Page {
  ...
  get returnAppButton() {
    return $("//*[@text='Return to App']");
  }
  get logOutAppButton() {
    return $("//*[@text='Log Out of App ']");
  }

  //get logOutAppButton () {return $('//android.view.View[@content-desc="Log Out of App "]')}

  get workspaceButton() {
    return $('//android.view.View[@content-desc="WORKSPACE"]');
  }
  get maximizeAppButton() {
    return $('//android.widget.Button[@content-desc="Maximize"]');
  }

  get chromeChoose() {
    return $(
      "//*[@resource-id='android:id/resolver_list']/android.widget.LinearLayout[1]"
    );
  }

  get alwaysChoose() {
    return $("//*[@resource-id='android:id/button_always']");
  }

  async chromeChoseClick() {
    await this.chromeChoose.waitForDisplayed({
  ...
  async alwaysClick() {
    await this.alwaysChoose.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async maximizeAppButtonClick() {
    await this.maximizeAppButton.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async signInHomeButtonClick() {
    await this.signInHomeButton.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async signInButtonIsDisplayed() {
    return await this.signInButton.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  }

  async returnAppButtonClick() {
    await this.returnAppButton.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async logOutButtonClick() {
    await this.logOutAppButton.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async workspaceButtonClick() {
    await this.workspaceButton.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
