...
class WelcomePage extends Page {
  ...
  get usernameField() {
    return $(
      '(//XCUIElementTypeOther[@name="main"])[1]/XCUIElementTypeTextField'
    );
  }

  get passwordField() {
    return $(
      '(//XCUIElementTypeOther[@name="main"])[1]/XCUIElementTypeSecureTextField'
    );
  }
  ...
  async neverSaveClick() {
    await this.neverSaveDialog.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async doneClick() {
    await this.doneBrowser.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async returnClick() {
    await this.returnToApp.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async signInHomeButtonClick() {
    await this.signInHomeButtoniOs.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
