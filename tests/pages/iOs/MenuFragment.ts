...
class MenuFragment extends Page {
  ...
  get returnButton() {
    return $(
      '//XCUIElementTypeOther[@name="FAIMS3"]/XCUIElementTypeOther/XCUIElementTypeButton'
    );
  }
  get menuButtonHome() {
  ...
  async menuButtonReturnClick() {
    await this.returnButton.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async menuButtonHomeClick() {
    await this.menuButtonHome.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async menuButtonWorkspaceClick() {
    await this.menuButtonWorkspace.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async menuButtonNoteBookClick() {
    await this.menuButtonNoteBooks.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async menuButtonNewNoteBookClick() {
    await this.menuButtonNewNotebook.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
  ...
  async menuButtonUserClick() {
    await this.menuButtonUser.waitForDisplayed({
      timeout: await this.getWaiterTimeForElement(),
    });
