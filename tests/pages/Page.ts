...
export default class Page {
  ...
  get idRoot() {
    return $("//android.view.View[@resource-id='root']");
  }
  get clickOutsideKeyboard() {
    return $('//XCUIElementTypeStaticText[@name="PROVIDER"]');
  }
  ...
  async pushFileToDevice() {
    const codingBot = readFileSync(
      join(process.cwd(), 'test/utils/sc1.png'),
      'base64'
    );
  ...
  async scrollToElement(element: WebdriverIO.Element) {
    await driver.touchAction([{action: 'moveTo', element: element}, 'release']);
  }
}
