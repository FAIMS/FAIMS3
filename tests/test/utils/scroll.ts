...
export const tapByCoordinates = async () => {
  await driver.touchAction({action: 'tap', x: 60, y: 1850});
};

export const tapByCoordinatesXY = async (x, y) => {
  await driver.touchAction({action: 'tap', x: x, y: y});
};

export const tapByCoordinatesIOS = async () => {
  await driver.touchAction({action: 'tap', x: 400, y: 960});
  if (deviceName.includes('iPhone')) {
    await driver.touchAction({action: 'tap', x: 190, y: 620});
  }
};
