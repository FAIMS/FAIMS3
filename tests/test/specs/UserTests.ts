...
describe('User suite', () => {
  it('User check page', async () => {
    ...
    assert.ok(
      await UserPage.getCurrentUserIsDisplayed(),
      'Current user not presented'
    );
  });
  ...
  it('Work space check', async () => {
    ...
    assert.ok(
      await WorkSpacePage.availableTabIsDisplayed(),
      'Work space not presented'
    );
  });
  ...
  it('LogOut check', async () => {
    ...
    assert.ok(
      await WelcomePage.signInButtonIsDisplayed(),
