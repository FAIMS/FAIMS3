#!/bin/bash

## This script updates the version string for
## all sub-packages in package.json.

# get the new version from the first argument
NEW_VERSION=$1

FILES=("package.json" "api/package.json" "app/package.json" "web/package.json" "dashboard/package.json" "library/data-model/package.json" "library/forms/package.json")

for FILE in "${FILES[@]}"; do
  if [ -f "$FILE" ]; then
    # Update the version in package.json
    jq --arg newVersion "$NEW_VERSION" '.version = $newVersion' "$FILE" > tmp.$$.json && mv tmp.$$.json "$FILE"
    
    echo "Updated $FILE to version $NEW_VERSION"
  else
    echo "File $FILE does not exist."
  fi
done

# Update version in the user docs conf.py
sed -e "s/release = \".*\"/release = \"$NEW_VERSION\"/" docs/user/conf.py > tmp.$$.py && mv tmp.$$.py docs/user/conf.py
echo "Updated docs/user/conf.py to version $NEW_VERSION"
