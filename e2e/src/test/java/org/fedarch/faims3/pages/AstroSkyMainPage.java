...
public class AstroSkyMainPage {
	...
	public void fillOutFormWithValidFields() {
	    ...
	    if (driver instanceof AndroidDriver) {
	    	...
	    	intField.sendKeys(AstroSkyMainPage.INTEGER);
	    }
	...
	public void validateLatLong() {
		...
		if (driver instanceof AndroidDriver) {
			...
			this.generatedLat = latVal;
			this.generatedLong = longVal;
		} else {
			...
			this.generatedLat = latitude;
			this.generatedLong = longitude;
		}
	}

	/**
	 * Make sure lat long values are the same as previously generated.
	 * @return
	 */
	public void checkLatLongValues() {
		// in Android 10, the element class name is actually android.widget.TextView
		// but otherwise it's android.view.View
		// ideally there should be an id so it's nice and easy, but we work with what
		// we've got
		// so we have to work out the class name and get the siblings
		if (driver instanceof AndroidDriver) {
			String className = lat.getAttribute("className");
			...
			String latVal = this.lat.getText();
			assertEquals(TestUtils.roundCoordinate(latVal),
					TestUtils.roundCoordinate(this.generatedLat));

			String longVal = this.longitude.getText();
			assertEquals(TestUtils.roundCoordinate(longVal),
					TestUtils.roundCoordinate(this.generatedLong));
		} else {
			String text = driver.findElement(By.xpath("//button/span[text()='Take Point']/../following-sibling::span"))
					.getText();
			//TODO: We may need to do some rounding

			String expectedText = new StringBuffer("Lat: ").append(this.generatedLat)
					.append("; Long: ").append(this.generatedLong)
					.toString();
			assertEquals(text, expectedText);
		}
