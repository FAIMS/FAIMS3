 ...
 *
 * Filename: TestIncompleteDraftObservationChrome.java
 * Description:
 *   TODO
 */
...
public class TestIncompleteDraftObservationChrome extends ChromeTest implements TestIncompleteDraftObservation {
	...
	public void testIncompleteDraft() throws Exception {
		try {
			//In Test Project “Astrosky” click + New Observation on both chrome and Chrome.
			loadNewAstroSkyForm();

			//Enter a sample email address into the form:
			//draft-observation-Chrome-testername@faims.edu.au on the Chrome
			WebDriverWait wait = new WebDriverWait(driver, 10);
			WebElement emailField = wait.until(
			        ExpectedConditions.presenceOfElementLocated((By.id("email-field"))));
		    emailField.sendKeys(AstroSky.EMAIL_DRAFT_CHROME);
		    ...
		    assertEquals(AstroSky.EMAIL_DRAFT_CHROME, driver.findElement(By.id("email-field")).getAttribute("value"));
	  } catch (Exception e) {
	  ...
	  TestUtils.markBrowserstackTestResult(driver, isUsingBrowserstack(), true, "Chrome - TestIncompleteDraftObservationChrome.testIncompleteDraft() passed!");
  }
}
