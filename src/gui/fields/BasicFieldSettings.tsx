...
const DefaultuiSpec = {
 ...
 const DefaultuiSetting:ProjectUIModel={
  ...
  const getfieldNamesbyView = (uiSetting:ProjectUIModel,view:string,fieldui:ProjectUIFields) =>{
    if(view==='meta')  return uiSetting['views'][view]['fields']??[];
    if(view==='validationSchema')  return uiSetting['views'][view]['fields']??[];
    if(view==='access')  return uiSetting['views'][view]['fields']??[];
    if(view==='FormParamater')  return uiSetting['views'][view]['fields']??[];
    if(uiSetting['views'][view]!==undefined&&fieldui['component-parameters'][view]!==undefined)
  ...
  export function Defaultcomponentsetting(props:componenentSettingprops)  {
    ...
    return (
      ...
      uiSetting['viewsets'][props.designvalue]['views'].length===0?'next':
      uiSetting['viewsets'][props.designvalue]['views'].map((view:any)=>
        getfieldNamesbyView(uiSetting,view,props.fieldui).map((fieldName: string) =>
