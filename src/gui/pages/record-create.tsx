...
export default function RecordCreate() {
  ...
  const [variant, setVariant] = useState(null as null | string);
  ...
  if (error !== null) {
    ...
    // Pick a variant after loading
    return (
      <Container maxWidth="lg">
        {uiSpec.default_variants.map(variant_name => {
          const variant_info = uiSpec.variants[variant_name];
          return (
            <Button
              variant="contained"
              color="primary"
              onClick={() => setVariant(variant_name)}
            >
              {variant_info.label || variant_name}
            </Button>
          );
        })}
      </Container>
