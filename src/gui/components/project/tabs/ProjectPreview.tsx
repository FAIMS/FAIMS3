...
export default function ProjectPreviewTab(props:ProjectPreviewProps) {
    ...
    const [formvariants,setFormVariants]= useState<string>(formuiSpec['visible_types'][0])
    ...
    const handleChangeFormRole = (event:any) => {
      ...
      const newfields=getfieldNames(formuiSpec['views'][formvariants+'SECTION1']['fields'],event.target.value)
  ...
  return (
    <>
        <TabTab tabs={formtabs} value={formtabs.indexOf(formlabel)>0?formtabs.indexOf(formlabel):0} handleChange={handelonChangeVariants}  tab_id='primarytab' />
        {formtabs.length>0?formtabs.map((tab:string,index:number)=>
            <TabPanel value={formtabs.indexOf(formlabel)>0?formtabs.indexOf(formlabel):0} index={index} tabname='primarytab' >
                    ...
                    <Formik
                        ...
                        {formProps => {
                            return (
                                ...
                                getComponentFromField(formuiSpec,fieldName,formProps,handleChangeForm))}
                                </Form>
                            );
                        }}
                ...
                <FormForm currentView={'start-view'} handleChangeForm={handleChangeFormRole} handleSubmit={handleSubmitForm} uiSpec={getprojectform(projectvalue,'preview',{forms:[formvariants]})} />
                </Grid>
                </Grid>
            </TabPanel>
