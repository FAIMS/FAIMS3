...
export async function check_if_record_relationship(relationA:Relationship|null|undefined,relationB:Relationship|null|undefined,project_id:string,record_id:string){
  ...
  let linked:any[] = []
  if(relationA!==null&&relationA!==undefined&&relationA.linked!==undefined) linked=relationA.linked
  if(relationB!==null&&relationB!==undefined&&relationB.linked!==undefined) linked=[...linked,relationB.linked]
  ...
  if(linked.length>0){
    ...
    for (const index in linked){
      ...
      if(!new_linked_array.includes(id)){
        ...
        const is_linked = await check_if_parent_link(link,linked[index].field_id,record_id)
        if(is_linked) new_relation['linked'] = [...new_relation['linked'],linked[index]]
      }
    }
  }
  return new_relation;
...
async function check_parent(relation:Relationship|null|undefined,project_id:string,record_id:string){
  ...
  if(relation.parent!==undefined){
    ...
    const is_linked = await check_if_parent_link(parent,relation.parent.field_id,record_id)
...
async function check_if_parent_link(record: RecordReference,field_id:string,record_id:string){
  let is_exist=false
  ...
  if(latest_record!==null){
      if(Array.isArray(latest_record.data[field_id])){
        ...
        latest_record.data[field_id].map(
          (child_record:RecordReference)=>{
            if(child_record.record_id===record_id)  
              is_exist=true}
        )
        return is_exist
      }else{
        if(latest_record.data[field_id].record_id===record_id)
         return true
      }  
  }
  return is_exist;
}
function check_if_child_link(relationship: Relationship|undefined,field_id:string,record_id:string,relation_type:string){
  let is_exist=false
  if(relationship===undefined) return false;
  if(relation_type==='Child'){
    if(relationship.parent===undefined||relationship.parent===null)
      return false
    if(relationship.parent.record_id===record_id&&relationship.parent.field_id===field_id)
      return true
    else return false // if parent is not this parent, then return false
  }else{
        //if link relationship check parent
    if(relationship.linked===undefined||relationship.linked===null||relationship.linked.length===0)
      return false
    relationship.linked.map(
          (record:LinkedRelation)=>{
...
export function get_all_child_records(conflictA: any,conflictB: any){
  if(conflictA===null||conflictA===''||(Array.isArray(conflictA)&&conflictA.length===0) ){
    if(conflictB===null||conflictB===''||(Array.isArray(conflictB)&&conflictB.length===0))
      return null
    else if(!Array.isArray(conflictB)) return [conflictB]
    else return conflictB
  }else if(conflictB===null||conflictB===''||(Array.isArray(conflictB)&&conflictB.length===0)){
