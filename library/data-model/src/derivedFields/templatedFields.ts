 ...
 * Description:
 *   Templated string evaluation: mustache rendering with system variables
 *   (_CREATOR_NAME, _CREATED_TIME) and parent values (_PARENT.<Field-ID>).
 *   Hoisted from @faims3/forms so the API's refresh pass (#2245) shares the
 *   exact evaluation the form runs on view. Field-type template functions are
 *   looked up via an injectable parameter defaulting to the builtin map;
 *   @faims3/forms passes its registry-backed lookup, which resolves to the
 *   same functions.
 */
